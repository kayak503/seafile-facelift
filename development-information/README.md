# Grapple Drive development information
...
Grapple Drive is a Next.js Backend-for-Frontend for Seafile. The browser talks only to Grapple routes. Grapple authenticates with Seafile, keeps the resulting account token on the server, and forwards authorized file operations to Seafile.
...
Browser
  ...
  └─ /api/* requests with an HttpOnly Grapple session
          │
          ▼
...
- `components/public-share-view.tsx` — recipient-facing Grapple share page
...
| `APP_URL`            | Yes          | Canonical Grapple origin used for links, secure-cookie behavior, and mutation-origin validation. |
...
| `GRAPPLE_IMAGE`      | Compose only | Container image tag or digest.                                                                   |
| `GRAPPLE_PORT`       | Compose only | Host port mapped to container port 3000.                                                         |
| `SEAFILE_NETWORK`    | Compose only | Existing external Docker network shared with Seafile.                                            |
...
| --------------------------- | ----------------------- | ------------------------------------------------------------------------- |
| `/api/auth/login`           | `POST`                  | Authenticate with Seafile and create an opaque Grapple session.           |
...
| `/api/public-share/unlock`  | `POST`                  | Validate a Grapple share password and issue a scoped cookie.              |
...
- `SESSION_SECRET` changes invalidate sessions and public Grapple share payloads.
...
2. Grapple Drive on port `3200`, configured to use that fake service.
...
```bash
npm run check
docker build --pull -t grapple-drive:local .
...
The final image runs as an unprivileged user, uses the Next.js standalone output, includes a health check, and does not require a writable application filesystem.
...
`.github/workflows/container.yml` publishes signed-provenance, SBOM-enabled `linux/amd64` and `linux/arm64` images to GitHub Container Registry. Changes on the default branch publish `main`, `sha-*`, and `latest` tags; tags such as `v1.2.3` also publish `1.2.3` and `1.2`. The repository package must be public for unauthenticated pulls shown in the root README.
...
5. `npm run test:e2e`
6. `npm run build`
7. `docker build --pull -t grapple-drive:<version> .`
8. Scan the image and production dependencies.
9. Push a versioned image and immutable digest.
10. Deploy to staging and verify login, file preview, upload, rename, search, share, logout, and `/api/health`.

The historical product audit is retained at [`AUDIT_REPORT.md`](AUDIT_REPORT.md). It is evidence from an earlier build, not a current defect list.
